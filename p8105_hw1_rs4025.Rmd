...
The data frame, 'df2', has `r nrow(df2)` rows and `r ncol(df2)` columns. The mean of x is about `r mean( pull(df2, x))`, the median of x is about `r median( pull(df2, x))`, and the standard deviation of x is about `r sd( pull(df2, x))`. The proportion of cases for which x+y > 1 is `r sum( pull(df2, log_var))/500`. 
